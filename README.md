...
New York, NY<br>
   ●   At Mango Concept we build apps and websites, drive traffic to them, and in the process collaborate with talented entrepreneurs and companies.  We also maintain, optimize and redesign exiting websites; I manage the Nyfa.org website and oversee the rebuilding and redesigning of it.<br>
   ...
   Technology and achievements:<br>
   ●   Wordpress - Built custom theme from underscores starter theme (https://underscores.me), utilized "sassify" option to include well organized .scss files for all starting styling.  Created Ruby script that calls Google Maps API to process store data from client so could import thousands of locations into "WP Store Locator" plugin.  Modified purchased themes and imported content.  Maintained existing Wordpress sites with minimal downtime.<br>
   ●   Ruby on Rails - Engineered album pre-save feature so fans could pre-save album to their Spotify account.  Developed multiple mobile-friendly Sass styled sites from designer mockup, with features such as: save newsletter signup to database and display flash alert, superimpose game screenshot over iPhone and iPad outlines.<br>
   ●   .NET - Implemented complete redesign on massive legacy .NET application.  Re-architected deployment system to simple Visual Studio export when bamboo cloud was discontinued.  Fixed Authorize.net integration when older TLS versions were discontinued by forcing the security protocol TLS 1.2.  Reduced AWS bill by scaling down servers after identifying and repairing costly database queries.<br>
